...
if ~exist(['fi_' num2str(digits-1) num2str(DarkFramePresent) '.mat'])
...
if strcmp(button,'Cancel'), return, end
if strcmp(button,'Yes')
    load(['fi_' num2str(digits-1) num2str(DarkFramePresent) '.mat']); % load filtered data for 1st frame (after darkframe)
    sze=size(fz);
...
load(['fi_' num2str(digits-1) num2str(DarkFramePresent) '.mat']); % load filtered data for 1st frame
