...
for pict=1:NumberOfFrames+DarkFramePresent   % main loop
    ...
    if pict == 1
     newsize=size(z); newymax=newsize(1); newxmax=newsize(2); % �berfl�ssig in Schleife - fz hat konstante Gr�sse
...
% Zeifacher Fit f�r den Fall, dass das Anfangs-D zu schlecht ist.
% Fit mit "echtem" Chiquadrat, daf�r: Fehler der Messpunkte aus Polynomfit
...
% pict=StartF;
% CBD=struct('pict',pict,'NofF',NofF,'tP1binx',tP1binx,'tI1binAvg',tI1binAvg,'xScale',xScale); % Callbacks der Kn�pfe ben�tigen diese Daten
% guidata(fig2hndl,CBD); % CallBackData in Datenbereich der figure
% evalin('base','CBD=guidata(gcf);'); % CBD im Base-Bereich f�r Callbacks erzeugen
...
sigma0=Pout(1);
offsetSF=Pout(3);
ActPos=Pout(2); % Distance from tip in �m
   % show the result
...
if model==1                    % Vollst�ndiges Modell mit Images, braucht Positionen von Tip, Body und Aktivierungsstelle
...
% B ist unabh�ngige, A ist abh�ngige Variable, also A(k)=f(B(k)). B-Werte sind nicht �quidistant.
% put2bins ordnet jeden B-Wert einem Intervall (=bin) der Breite binWidth auf einer �quidistanten Skala zu.
% Diese Skala l�uft von min(B) bis max(B) in Schritten von binWidth und
% wird in Bbin zur�ckgegeben. Die zu einem Intervall j geh�renden A-Werte
...
% d.h. nB enth�lt i.A. nicht jede Zahl von
% 1 bis nBstop
...
for k=kB                           % Schleife vektorisieren = ?
    ...
    if ~isempty(binmembers)           % Mittelwert �ber Bin-Mitglieder
...
z=(x-zentr)/skala;
[p,S]=polyfit(z,y,4);
[Y,dev]=polyval(p,z,S); % dev enth�lt die Fehler (im statistischen Sinn, s. Matlab-Hilfe)
...
sigma0=P(1);lambda=HP(4); %vollst�ndiges Modell mit Images
